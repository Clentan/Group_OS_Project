...
def get_sim() -> Simulation:
    if "sim" not in st.session_state:
        st.session_state.sim = Simulation(threads=10, resources=6)
...
def last_events(sim: Simulation, limit=50, only_cycle: Optional[Set[str]] = None):
    with sim.logger.lock:
        items = sim.logger.buffer[-400:]
    rows = []
    for rec in items:
...
if st.sidebar.button("Reset", use_container_width=True):
    if sim.running:
        sim.stop()
    st.session_state.sim = Simulation(threads=threads, resources=resources)
...
# Deadlock status messages (resolved vs detected) + victim policy/explanation
if sim.pending_deadlock:
    ...
    # Show "Deadlock resolved" briefly (5 seconds), then clear
...
with left:
    ...
    st.caption("Auto-refreshes while running.")

with right:
    st.subheader("System Log")
    only_cycle = set(sim.pending_deadlock["nodes"]) if sim.pending_deadlock else None
    show_cycle_only = st.checkbox("Show only processes in detected cycle", value=bool(only_cycle))
    rows = last_events(sim, limit=50, only_cycle=only_cycle if show_cycle_only else None)
...
# Keep the sidebar policy in session state so right pane can read it
if sim.pending_deadlock:
    # This selectbox key must match the earlier selectbox to sync state
    pass
